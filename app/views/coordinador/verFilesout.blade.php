...
@section('titulo')
         {{ Auth::userCoord()->get()->Nombre.' '.Auth::userCoord()->get()->Apellidos }} | Archivos enviados
@endsection
...
<div class="content"><!-- div content (general) -->
    <div class="container-fluid"><!-- div container-fluid -->
        ...
        {{ Form::open(['route' => 'viewFilesOut.coordinador', 'method' => 'GET', 'class' => 'pull-right', 'role' => 'search']) }}
                ...
                @foreach($outFiles as $outFile)
                    <tr>
                        <td style="color: #00768B;">
                            <a href="{{ route('detalleArchivo.coordinador', [$outFile->Id_File]) }}" 
                                        title="Ver archivo" style="cursor:pointer; cursor: hand">
                                {{ $outFile->clientOriginalName }}
                            </a>
                        </td>                       
                        <td>
                            <a href="{{ route('detalleArchivo.coordinador', [$outFile->Id_File]) }}" 
                        ...
                        <td><center>{{ $outFile->NombreCliente() }}</center></td>
                        <td><center>{{ $outFile->created_at }}</center></td>
                        <td>
                            <a href="{{ route('FileDeleted.Coordinador', [$outFile->Id_File]) }}" title="Eliminar archivo" style="cursor:pointer; cursor: hand" onclick="return confirm('¿Seguro de eliminar el archivo?')">
                                <font color="#D00000">
